...
4. Write the handoff to a temporary file (e.g., `.kt/tmp.md`), then pipe it to the engine to avoid PowerShell encoding issues: `Get-Content .kt/tmp.md | python $HOME/.kt/kt.py save --tool "Antigravity" --note "<headline>"` (then delete the temp file).
